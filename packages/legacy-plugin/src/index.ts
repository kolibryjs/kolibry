...
import { build, normalizePath } from 'kolibry'
...
// Duplicated from build.ts in Kolibry Core, at least while the feature is experimental
...
const legacyEnvVarMarker = '__KOLIBRY_IS_LEGACY__'

const _require = createRequire(import.meta.url)

function kolibryLegacyPlugin(options: Options = {}): Plugin[] {
   ...
   const isDebug
    = debugFlags.includes('kolibry:*') || debugFlags.includes('kolibry:legacy')
   ...
   const legacyConfigPlugin: Plugin = {
      ...
      config(config, env) {
         if (env.command === 'build' && !config.build?.ssr) {
            ...
            if (genLegacy) {
               // Kolibry's default target browsers are **not** the same.
      ...
      configResolved(config) {
         if (overriddenBuildTarget) {
            config.logger.warn(
               colors.yellow(
                  'legacy-plugin overrode \'build.target\'. You should pass \'targets\' as an option to this plugin with the list of legacy browsers to support instead.',
               ),
            )
         }
      },
   ...
   const legacyGenerateBundlePlugin: Plugin = {
      name: 'kolibry:legacy-generate-polyfill-chunk',
      apply: 'build',
      ...
      async generateBundle(opts, bundle) {
         ...
         if (!isLegacyBundle(bundle, opts)) {
            ...
            isDebug
          && consolji.log(
             '[@kolibryjs/legacy-plugin] modern polyfills:',
             modernPolyfills,
          )
         ...
         if (legacyPolyfills.size) {
            ...
            isDebug
          && consolji.log(
             '[@kolibryjs/legacy-plugin] legacy polyfills:',
             legacyPolyfills,
          )
   ...
   const legacyPostPlugin: Plugin = {
      name: 'kolibry:legacy-post-process',
      enforce: 'post',
      apply: 'build',

      configResolved(_config) {
         if (_config.build.lib) {
            throw new Error('@kolibryjs/legacy-plugin does not support library mode.')
         }
         config = _config
        ...
        || 'last 2 versions and not dead, > 0.3%, Firefox ESR'
         isDebug && consolji.log('[@kolibryjs/legacy-plugin] targets:', targets)
...
const polyfillId = '\0kolibry/legacy-polyfills'
...
function recordAndRemovePolyfillBabelPlugin(
   ...
   return ({ types: t }: { types: typeof BabelTypes }): BabelPlugin => ({
      name: 'kolibry-remove-polyfill-import',
      post({ path }) {
...
function replaceLegacyEnvBabelPlugin(): BabelPlugin {
   return ({ types: t }): BabelPlugin => ({
      name: 'kolibry-replace-env-legacy',
      visitor: {
         Identifier(path) {
...
export default kolibryLegacyPlugin
