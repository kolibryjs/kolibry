...
const debug = createDebugger('kolibry:config')
...
/**
 * Type helper to make it easier to use kolibry.config.ts
...
export interface UserConfig {
   ...
   * deps or some other cache files that generated by kolibry, which can improve
   * the performance. You can use `--force` flag or manually delete the directory
   * to regenerate the cache files. The value can be either an absolute file
   * system path or a path relative to project root.
   * Default to `.kolibry` when no `package.json` is detected.
   * @default 'node_modules/.kolibry'
   */
  cacheDir?: string
  /**
  ...
  /**
   * Array of kolibry plugins to use.
   */
   ...
   * Please be careful and always pin Kolibry's version when using them.
   * @experimental
   */
   ...
   * future minor version. Please always pin Kolibry's version to a minor when using them.
   */
  ...
  worker?: {
    ...
    format?: 'es' | 'iife'
    /**
     * Kolibry plugins that apply to worker bundle
     */
    plugins?: PluginOption[]
    /**
...
export interface ExperimentalOptions {
  ...
  /**
   * Skips SSR transform to make it easier to use Kolibry with Node ESM loaders.
   * @warning Enabling this will break normal operation of Kolibry's SSR in development mode.
   *
   * @experimental
   * @default false
   */
...
export interface LegacyOptions {
  /**
   * Revert kolibry build --ssr to the v2.9 strategy. Use CJS SSR build and v2.9 externalization heuristics
   *
   * @experimental
   * @deprecated
   * @default false
...
export async function resolveConfig(
  ...
  // So we need to separate the worker plugin from the plugin that kolibry needs to run.
  ...
  if (
    ...
    process.env.KOLIBRY_TEST_WITHOUT_PLUGIN_COMMONJS
  ) {
  ...
  // loaded from `.staging.env` and set by us as KOLIBRY_USER_NODE_ENV
  const userNodeEnv = process.env.KOLIBRY_USER_NODE_ENV
    ...
    if (userNodeEnv === 'development') {
      ...
      logger.warn(
        `NODE_ENV=${userNodeEnv} is not supported in the .env file. ` +
          ...
          `If you need to set process.env.NODE_ENV, you can set it in the Kolibry config instead.`,
      )
    }
  }
  ...
  const cacheDir = normalizePath(
    config.cacheDir
      ...
      : pkgDir
      ? path.join(pkgDir, `node_modules/.kolibry`)
      : path.join(resolvedRoot, `.kolibry`),
  )
    ...
    logger.warn(
      colors.yellow(
        `build.terserOptions is specified but build.minify is not set to use Terser. ` +
          `Note Kolibry now defaults to use esbuild for minification. If you still ` +
    ...
    if (assetFileNamesList.length > 1) {
      ...
      if (hasDifferentReference) {
        resolved.logger.warn(
          colors.yellow(`
assetFileNames isn't equal for every build.rollupOptions.output. A single pattern across all outputs is supported by Kolibry.
`),
        )
      }
    }
...
/**
 * Resolve base url. Note that some users use Kolibry to build for non-web targets like
...
export function resolveBaseUrl(
  ...
  if (!isBuild || !isExternal) {
    base = new URL(base, 'http://kolibry.zip').pathname
...
async function bundleConfigFile(
  ...
  const dirnameVarName = '__kolibry_injected_original_dirname'
  const filenameVarName = '__kolibry_injected_original_filename'
  const importMetaUrlVarName = '__kolibry_injected_original_import_meta_url'
