 ...
 * kolibry-specific options. A valid kolibry plugin is also a valid Rollup plugin.
 * On the contrary, a Rollup plugin may or may NOT be a valid kolibry universal
...
export interface Plugin extends RollupPlugin {
   ...
   * - alias resolution
   * - `enforce: 'pre'` plugins
   * - kolibry core plugins
   * - normal plugins
   * - kolibry build plugins
  ...
  >
  /**
   * Configure the kolibry server. The hook receives the {@link KolibryDevServer}
   ...
   *
   * - html: string
   * - ctx?: kolibry.ServerContext (only present during serve)
   * - bundle?: rollup.OutputBundle (only present during build)
   *
   * It can either return a transformed string, or a list of html tag
   * descriptors that will be injected into the `<head>` or `<body>`.
   *
   * By default the transform is applied **after** kolibry's internal html
