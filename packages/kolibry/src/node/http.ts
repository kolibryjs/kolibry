...
export interface CommonServerOptions {
  /**
   * Specify server port. Note if the port is already being used, Kolibry will
   * automatically try the next available port so this may not be the actual
   * port the server ends up listening on.
   */
  port?: number
  /**
   * If enabled, kolibry will exit if specified port is already in use
   */
  strictPort?: boolean
  /**
      ...
      logger.warn(
        colors.yellow(
          'Server responded with status code 431. ' +
            'See https://kolibry.zip/guide/troubleshooting.html#_431-request-header-fields-too-large.',
        ),
      )
    }
