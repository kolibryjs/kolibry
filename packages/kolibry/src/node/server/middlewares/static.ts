...
function sirvOptions({
   ...
   return {
      ...
      // these files to be TypeScript files, and for Kolibry to serve them with
      // this Content-Type.
...
export function servePublicMiddleware(
   ...
   return function kolibryServePublicMiddleware(req, res, next) {
      // skip import request and internal requests `/@fs/ /@kolibry-client` etc...
...
export function serveStaticMiddleware(
   ...
   // Keep the named function. The name is visible in debug logs via `DEBUG=connect:dispatcher ...`
   return function kolibryServeStaticMiddleware(req, res, next) {
      // only serve the file if it's not an html request or ends with `/`
      // so that html requests can fallthrough to our html middleware for
      // special processing
      // also skip internal requests `/@fs/ /@kolibry-client` etc...
...
export function serveRawFsMiddleware(
   ...
   // Keep the named function. The name is visible in debug logs via `DEBUG=connect:dispatcher ...`
   return function kolibryServeRawFsMiddleware(req, res, next) {
      const url = new URL(req.url!, 'http://example.com')
...
function ensureServingAccess(
   ...
   if (isFileReadable(cleanUrl(url))) {
      const urlMessage = `The request url "${url}" is outside of Kolibry serving allow list.`
      const hintMessage = `
${server.config.server.fs.allow.map(i => `- ${i}`).join('\n')}

Refer to docs https://kolibry.zip/config/server-options.html#server-fs-allow for configurations and more details.`
