...
export const debugHmr = createDebugger('kolibry:hmr')
...
export async function handleHMRUpdate(
   ...
   const isConfigDependency = config.configFileDependencies.includes(
      file,
   )
   const isEnv
...
export function updateModules(
   ...
   modules: ModuleNode[],
   timestamp: number,
   { config, ws, moduleGraph }: KolibryDevServer,
...
// kolibryjs/kolibry#610 when hot-reloading Vue files, we read immediately on file
