...
export interface ServerOptions extends CommonServerOptions {
  ...
  watch?: WatchOptions
  /**
   * Create Kolibry dev server to be used as a middleware in an existing server
   * @default false
   */
  middlewareMode?: boolean | 'html' | 'ssr'
  /**
   * Prepend this folder to http requests, for use when proxying kolibry as a subfolder
...
export interface KolibryDevServer {
  /**
   * The resolved kolibry config object
   */
  ...
  /**
   * The resolved urls Kolibry prints on the CLI. null in middleware mode or
  ...
  /**
   * Apply kolibry built-in HTML transforms and any plugin HTML transforms.
   */
  transformIndexHtml(
    url: string,
    html: string,
   ...
   * Start the server.
   */
  listen(port?: number, isRestart?: boolean): Promise<KolibryDevServer>
  /**
   * Stop the server.
   */
  close(): Promise<void>
...
export async function _createServer(
  ...
  ws.on('kolibry:invalidate', async ({ path, message }: InvalidatePayload) => {
  ...
  middlewares.use(function kolibryHMRPingMiddleware(req, res, next) {
    if (req.headers['accept'] === 'text/x-kolibry-ping') {
  ...
  if (config.appType === 'spa' || config.appType === 'mpa') {
    ...
    // handle 404s
    // Keep the named function. The name is visible in debug logs via `DEBUG=connect:dispatcher ...`
    middlewares.use(function kolibry404Middleware(_, res) {
      res.statusCode = 404
      res.end()
    })
  }
...
export function resolveServerOptions(
  ...
  // only push client dir when kolibry itself is outside-of-root
...
async function restartServer(server: KolibryDevServer) {
  global.__kolibry_start_time = performance.now()
...
async function updateCjsSsrExternals(server: KolibryDevServer) {
