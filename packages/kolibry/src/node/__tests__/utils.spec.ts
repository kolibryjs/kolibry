...
describe('bareImportRE', () => {
   test('should work with normal package name', () => {
      expect(bareImportRE.test('kolibry')).toBe(true)
   })
   test('should work with scoped package name', () => {
      expect(bareImportRE.test('@kolibryjs/plugin-vue')).toBe(true)
   })
...
describe('injectQuery', () => {
   if (isWindows) {
      ...
      test('normalize windows path', () => {
         expect(injectQuery('C:\\User\\Kolibry\\Project', 'direct')).toEqual(
            'C:/User/Kolibry/Project?direct',
         )
      })
   ...
   test('relative path', () => {
      expect(injectQuery('usr/kolibry/%20a%20', 'direct')).toEqual(
         'usr/kolibry/%20a%20?direct',
      )
      expect(injectQuery('./usr/kolibry/%20a%20', 'direct')).toEqual(
         './usr/kolibry/%20a%20?direct',
      )
   ...
   test('path with hash', () => {
      expect(injectQuery('/usr/kolibry/path with space/#1?2/', 'direct')).toEqual(
         '/usr/kolibry/path with space/?direct#1?2/',
      )
   })

   test('path with protocol', () => {
      expect(injectQuery('file:///usr/kolibry/%20a%20', 'direct')).toMatch(
         'file:///usr/kolibry/%20a%20?direct',
      )
      expect(injectQuery('http://usr.kolibry/%20a%20', 'direct')).toMatch(
         'http://usr.kolibry/%20a%20?direct',
      )
   })

   test('path with multiple spaces', () => {
      expect(injectQuery('/usr/kolibry/path with space', 'direct')).toEqual(
         '/usr/kolibry/path with space?direct',
      )
   })

   test('path with multiple % characters', () => {
      expect(injectQuery('/usr/kolibry/not%20a%20space', 'direct')).toEqual(
         '/usr/kolibry/not%20a%20space?direct',
      )
   })

   test('path with %25', () => {
      expect(injectQuery('/usr/kolibry/%25hello%25', 'direct')).toEqual(
         '/usr/kolibry/%25hello%25?direct',
      )
   })

   test('path with Unicode', () => {
      expect(injectQuery('/usr/kolibry/東京', 'direct')).toEqual(
         '/usr/kolibry/東京?direct',
      )
   })

   test('path with Unicode, space, and %', () => {
      expect(injectQuery('/usr/kolibry/東京 %20 hello', 'direct')).toEqual(
         '/usr/kolibry/東京 %20 hello?direct',
      )
   })
})
