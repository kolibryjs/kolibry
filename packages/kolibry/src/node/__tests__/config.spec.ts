...
describe('resolveEnvPrefix', () => {
   test('use \'KOLIBRY_\' as default value', () => {
      const config: UserConfig = {}
      expect(resolveEnvPrefix(config)).toMatchObject(['KOLIBRY_'])
   })
...
describe('resolveConfig', () => {
   const keepScreenMergePlugin = (): PluginOption => {
      return {
         name: 'kolibry-plugin-keep-screen-merge',
         config() {
   ...
   const keepScreenOverridePlugin = (): PluginOption => {
      return {
         name: 'kolibry-plugin-keep-screen-override',
         config(config) {
