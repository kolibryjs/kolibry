...
// TODO: merge with PreviewServer in next major
export interface PreviewServerForHook {
   ...
   */
   ...
   /**
   * The resolved urls Kolibry prints on the CLI
   */
...
/**
 * Starts the Kolibry server in preview mode, to simulate a production deployment
 */
export async function preview(
   ...
   if (
    ...
    && process.argv[1]?.endsWith(path.normalize('bin/kolibry.js'))
   ...
   const kolibryAssetMiddleware = (...args: readonly [any, any?, any?]) =>
      sirv(distDir, {
         etag: true,
         dev: true,
   ...
   app.use(previewBase, kolibryAssetMiddleware)
