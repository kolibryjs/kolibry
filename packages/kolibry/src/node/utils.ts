...
import type WizardString from 'wizard-string'
...
import type { KolibryDevServer, ResolvedServerUrls } from './server'
...
// set in bin/kolibry.js
const filter = process.env.KOLIBRY_DEBUG_FILTER
...
export type KolibryDebugScope = `kolibry:${string}`

export function createDebugger(
   namespace: KolibryDebugScope,
...
export function diffDnsOrderChange(
   oldUrls: KolibryDevServer['resolvedUrls'],
   newUrls: KolibryDevServer['resolvedUrls'],
): boolean {
   return !(
      oldUrls === newUrls
      || (oldUrls
...
export function requireResolveFromRootWithFallback(root: string,
   id: string): string {
