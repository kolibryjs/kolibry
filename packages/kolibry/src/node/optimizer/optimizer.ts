...
async function createDepsOptimizer(
   ...
   function registerMissingImport(
      ...
      // We'll wait until the user codebase is eagerly processed by Kolibry so
      ...
      if (!crawlEndFinder) {
         if (isBuild) {
            logger.error(
               'Kolibry Internal Error: Missing dependency found after crawling ended',
            )
         }
...
async function createDevSsrDepsOptimizer(
   ...
   const depsOptimizer = {
      ...
      registerMissingImport: () => {
         throw new Error(
            'Kolibry Internal Error: registerMissingImport is not supported in dev SSR',
         )
      },
