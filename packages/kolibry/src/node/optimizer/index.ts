...
const debug = createDebugger('kolibry:deps')
...
export interface DepOptimizationConfig {
   ...
   * with Kolibry's dep optimization.
   *
   * - `external` is also omitted, use Kolibry's `optimizeDeps.exclude` option
   * - `plugins` are merged with Kolibry's dep plugin
   *
   ...
   *
   * By default, Kolibry can optimize `.mjs`, `.js`, `.ts`, and `.mts` files. This option
...
export type DepOptimizationOptions = DepOptimizationConfig & {
   /**
   * By default, Kolibry will crawl your `index.html` to detect dependencies that
   * need to be pre-bundled. If `build.rollupOptions.input` is specified, Kolibry
   * will crawl those entry points instead.
   *
   * If neither of these fit your needs, you can specify custom entries using
   * this option - the value should be a fast-glob pattern or array of patterns
   * (https://github.com/mrmlnc/fast-glob#basic-syntax) that are relative from
   * kolibry project root. This will overwrite default entries inference.
   */
 ...
 * Used by Kolibry CLI when running `kolibry optimize`.
 */
...
/**
 * Internally, Kolibry uses this function to prepare a optimizeDeps run. When Kolibry starts, we can get
...
async function prepareEsbuildOptimizerRun(
   ...
   // time we replace it by __kolibry_process_env_NODE_ENV. This placeholder will be
   // later replaced by the define plugin
   const define = {
      'process.env.NODE_ENV': isBuild
         ? '__kolibry_process_env_NODE_ENV'
   ...
   if (isBuild) {
      ...
      if (rollupOptionsExternal) {
        ...
        || rollupOptionsExternal.some(ext => typeof ext !== 'string')
            ...
            throw new Error(
               '[kolibry] \'build.rollupOptions.external\' can only be an array of strings or a string when using esbuild optimization at build time.',
            )
         }
         ...
         if (!deps[normalizedId] && filter?.(normalizedId) !== false) {
            ...
            if (entry) {
               if (isOptimizable(entry, optimizeDeps)) {
                  if (!entry.endsWith('?__kolibry_skip_optimization'))
...
export function createIsOptimizedDepUrl(
   ...
   const depsCacheDirPrefix = depsCacheDirRelative.startsWith('../')
      /* If the cache directory is outside the root, the URL prefix would be something
         like '/@fs/absolute/path/to/node_modules/.kolibry' */
      ? `/@fs/${removeLeadingSlash(normalizePath(depsCacheDir))}`
      /* If the cache directory is inside the root, the URL prefix would be something
         like '/node_modules/.kolibry' */
