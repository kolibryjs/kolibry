...
const ignoreFlagRE = /\/\*\s*@kolibry-ignore\s*\*\//
...
function parseWorkerOptions(
  ...
  try {
    ...
    throw err(
      'Kolibry is unable to parse the worker options as the value is not static.' +
        'To ignore this error, please use /* @kolibry-ignore */ in the worker options.',
      optsStartIndex,
    )
  }
...
function getWorkerType(raw: string, clean: string, i: number): WorkerType {
  ...
  const hasKolibryIgnore = ignoreFlagRE.test(workerOptString)
  if (hasKolibryIgnore) {
    return 'ignore'
  }
...
export function workerImportMetaUrlPlugin(config: ResolvedConfig): Plugin {
  ...
  return {
    name: 'kolibry:worker-import-meta-url',
