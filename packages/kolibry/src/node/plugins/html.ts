...
const inlineCSSRE = /__KOLIBRY_INLINE_CSS__([a-z\d]{8}_\d+)__/g
...
// PS: key like `hash(/kolibry/playground/assets/index.html)_1`)
...
export function htmlInlineProxyPlugin(config: ResolvedConfig): Plugin {
   ...
   return {
      name: 'kolibry:html-inline-proxy',
...
export function overwriteAttrValue(
   ...
   if (!valueStart) {
      ...
      throw new Error(
         '[kolibry:html] internal error, failed to overwrite attribute value',
      )
   }
              ...
              && prop.value.includes('url('), // only url(...) in css need to emit file
               ...
               if (inlineStyle) {
                  ...
                  overwriteAttrValue(
                     s,
                     sourceCodeLocation,
              `__KOLIBRY_INLINE_CSS__${hash}_${inlineModuleIndex}__`,
                  )
               }
