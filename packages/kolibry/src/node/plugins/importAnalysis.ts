...
import WizardString from 'wizard-string'
...
import type { KolibryDevServer } from '..'
import {
   CLIENT_DIR,
   CLIENT_PUBLIC_PATH,
   DEP_VERSION_RE,
...
const debug = createDebugger('kolibry:import-analysis')

const clientDir = normalizePath(CLIENT_DIR)

const skipRE = /\.(?:map|json)(?:$|\?)/
export function canSkipImportAnalysis(id: string): boolean {
   return skipRE.test(id) || isDirectCSSRequest(id)
}
...
const hasKolibryIgnoreRE = /\/\*\s*@kolibry-ignore\s*\*\//
...
export function importAnalysisPlugin(config: ResolvedConfig): Plugin {
   ...
   function getEnv(ssr: boolean) {
      ...
      return _env.replace('"__kolibry__ssr__"', `${ssr}`)
   }

   return {
      name: 'kolibry:import-analysis',
      ...
      // running src/node/server/__tests__/pluginContainer.spec.ts
         ...
         try {
            ...
            const msg = isVue
               ? 'Install @kolibryjs/plugin-vue to handle .vue files.'
               : maybeJSX
                  ? isJsx
         ...
         let s: WizardString | undefined
         const str = () => s || (s = new WizardString(source))
         ...
         const normalizeUrl = async (
            ...
            if (moduleListContains(optimizeDeps?.exclude, url)) {
               if (depsOptimizer) {
                  ...
                  // because starting from node_modules/.kolibry will not find the dependency if it was not hoisted
         ...
         await Promise.all(
            imports.map(async (importSpecifier, index) => {
              ...
              && !isModuleCSSRequest(resolvedId)
                ...
                && !sourceExp.includes('__kolibry_glob_') // glob handles deprecation message itself
                     ) {
                        const newImport
                ...
                && !resolvedId.match(optimizedDepChunkRE)
                        ...
                        if (needsInterop === undefined) {
                           ...
                           // a dynamic import, then it is an internal Kolibry error
                           if (!file.match(optimizedDepDynamicRE)) {
                              config.logger.error(
                                 colors.red(
                        `Kolibry Error, ${url} optimized info should be defined`,
                                 ),
                              )
                           }
                        }
              ...
              && config.server.preTransformRequests
                  ...
                  if (!isInNodeModules(importer)) {
                     // check @kolibry-ignore which suppresses dynamic import warning
                     const hasKolibryIgnore = hasKolibryIgnoreRE.test(
                     ...
                     colors.reset(generateCodeFrame(source, start))
                       ...
                       '\nThe above dynamic import cannot be analyzed by Kolibry.\n'
                        + `See ${colors.blue(
                          'https://github.com/rollup/plugins/tree/master/packages/dynamic-import-vars#limitations',
                        )} `
                        + 'for supported dynamic import formats. '
                        + 'If this is intended to be left as-is, you can use the '
                        + '/* @kolibry-ignore */ comment inside the import() call to suppress this warning.\n',
                    )}`,
                        )
                     }
                  }
                ...
                || isExplicitImportRequired(url.slice(1, -1))
                  ...
                  `__kolibry__injectQuery(${url}, 'import')`,
                  { contentOnly: true },
                        )
                     }
                  }
         ...
         if (hasHMR && !ssr) {
          ...
          `${
            ...
            // inject hot context
            str().prepend(
          `import { createHotContext as __kolibry__createHotContext } from "${clientPublicPath}";`
            + `import.meta.hot = __kolibry__createHotContext(${JSON.stringify(
         ...
         if (needQueryInjectHelper) {
            str().prepend(
          `import { injectQuery as __kolibry__injectQuery } from "${clientPublicPath}";`,
            )
         }
...
interface ImportNameSpecifier { importedName: string; localName: string }
    ...
    || node.type === 'ExportNamedDeclaration'
      ...
      for (const spec of node.specifiers) {
        ...
        && spec.exported.type === 'Identifier'
            ...
            `__kolibry__cjsExportDefault_${importIndex}`,
               )
               importNames.push({ importedName, localName: defaultExports })
            }
            else {
               const localName = makeLegalIdentifier(
            `__kolibry__cjsExport_${exportedName}`,
               )
               importNames.push({ importedName, localName })
      ...
      `__kolibry__cjsImport${importIndex}_${rawUrl}`,
      )
