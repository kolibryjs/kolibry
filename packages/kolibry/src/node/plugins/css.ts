...
import type { KolibryDevServer, ResolveFn } from '../'
...
// const debug = createDebugger('kolibry:css')
...
export function cssPostPlugin(config: ResolvedConfig): Plugin {
      ...
      if (!assetFileNames) {
         ...
         return path.dirname(
            assetFileNames({
               name: cssAssetName,
               type: 'asset',
               source: '/* kolibry internal call, ignore */',
            }),
         )
      }
   }
   ...
   return {
      ...
      async transform(css, id, options) {
        ...
        && dataToEsm(modules, { namedExports: true, preferConst: true })
         ...
         if (config.command === 'serve') {
            ...
            const code = [
          `import { updateStyle as __kolibry__updateStyle, removeStyle as __kolibry__removeStyle } from ${JSON.stringify(
            path.posix.join(config.base, CLIENT_PUBLIC_PATH),
          )}`,
          `const __kolibry__id = ${JSON.stringify(id)}`,
          `const __kolibry__css = ${JSON.stringify(cssContent)}`,
          '__kolibry__updateStyle(__kolibry__id, __kolibry__css)',
          // css modules exports change on edit so it can't self accept
          `${
            modulesCode
            || 'import.meta.hot.accept()\nexport default __kolibry__css'
          }`,
          'import.meta.hot.prune(() => __kolibry__removeStyle(__kolibry__id))',
            ].join('\n')
         ...
         // and then use the cache replace inline-style-flag when `generateBundle` in kolibry:build-html plugin
      ...
      async renderChunk(code, chunk, opts) {
         ...
         const resolveAssetUrlsInCss = (
          ...
          = (encodedPublicUrls || relative)
            ...
            chunkCSS = chunkCSS.replace(assetUrlRE, (_, fileHash, postfix = '') => {
               ...
               chunk.kolibryMetadata!.importedAssets.add(cleanUrl(filename))
         ...
         if (config.build.cssCodeSplit) {
            ...
            if (opts.format === 'es' || opts.format === 'cjs') {
               ...
               chunk.kolibryMetadata!.importedCss.add(this.getFileName(referenceId))
               ...
               // Entry chunk CSS will be collected into `chunk.kolibryMetadata.importedCss`
               // and injected later by the `'kolibry:build-html'` plugin into the `index.html`
      ...
      augmentChunkHash(chunk) {
         if (chunk.kolibryMetadata?.importedCss.size) {
            let hash = ''
            for (const id of chunk.kolibryMetadata.importedCss)
               hash += id
      ...
      // @ts-expect-error asset emits are skipped in legacy bundle
         if (opts.__kolibry_skip_asset_emit__)
            return
                  ...
                  chunk.imports = chunk.imports.filter((file) => {
                     if (pureCssChunkNames.includes(file)) {
                        ...
                        importedCss.forEach(file =>
                           chunk.kolibryMetadata!.importedCss.add(file),
                        )
                        return false
                     }
                     return true
   ...
   if (needInlineImport) {
      postcssPlugins.unshift(
         (await importPostcssImport()).default({
            ...
            nameLayer(index) {
               return `kolibry--anon-layer-${getHash(id)}-${index}`
            },
         }),
      )
   }
         ...
         if (message.type === 'dependency') {
          ...
          = `${glob.escapePath(normalizePath(path.resolve(path.dirname(id), dir)))
           ...
           globPattern}`
            ...
            let msg = `[kolibry:css] ${message.text}`
...
async function resolvePostcssConfig(
   ...
   // inline postcss config via kolibry config
      ...
      result = postcssrc({}, searchPath).catch((e) => {
         if (!/No PostCSS Config found/.test(e.message)) {
            if (e instanceof Error) {
               ...
               throw new TypeError(`Failed to load PostCSS config: ${e}`)
            }
         }
         return null
      })
...
const UrlRewritePostcssPlugin: PostCSS.PluginCreator<{
   ...
   return {
      postcssPlugin: 'kolibry-url-rewrite',
      Once(root) {
...
const scss: SassStylePreprocessor = async (
   ...
   try {
      ...
      const deps = result.stats.includedFiles.map((f: string) => cleanScssBugUrl(f))
...
const less: StylePreprocessor = async (source, root, options, resolvers) => {
   ...
   const kolibryResolverPlugin = createKolibryLessPlugin(
      nodeLess,
      options.filename,
      options.alias,
      resolvers,
   ...
   try {
      result = await nodeLess.render(content, {
         ...options,
         plugins: [kolibryResolverPlugin, ...(options.plugins || [])],
         ...(options.enableSourcemap
            ? {
                  sourceMap: {
 ...
 */
let KolibryLessManager: any

function createKolibryLessPlugin(
   less: typeof Less,
   rootFile: string,
   alias: Alias[],
   resolvers: CSSAtImportResolvers,
): Less.Plugin {
   if (!KolibryLessManager) {
      KolibryLessManager = class KolibryManager extends less.FileManager {
         resolvers
         rootFile
         alias
         constructor(
   ...
   return {
      install(_, pluginManager) {
         pluginManager.addFileManager(
            new KolibryLessManager(rootFile, resolvers, alias),
         )
      },
      minVersion: [3, 0, 0],
   }
