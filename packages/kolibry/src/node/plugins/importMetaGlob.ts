...
import { stringifyQuery } from 'url-ops'
...
export function importGlobPlugin(config: ResolvedConfig): Plugin {
   let server: KolibryDevServer | undefined

   return {
      name: 'kolibry:import-glob',
...
function parseGlobOptions(
   ...
   try {
      ...
      throw err(
         'Kolibry is unable to parse the glob options as the value is not static',
         optsStartIndex,
      )
   }
...
const importPrefix = '__kolibry_glob_'

const { basename, dirname, relative, join } = posix

const warnedCSSDefaultImportVarName = '__kolibry_warned_css_default_import'
function jsonStringifyInOneline(input: any) {
   return JSON.stringify(input).replace(/[{,:]/g, '$& ').replace(/\}/g, ' }')
}
function createCssDefaultImportWarning(globs: string[],
   options: GeneralImportGlobOptions) {
   return `if (!${warnedCSSDefaultImportVarName}) {`
...
export function isVirtualModule(id: string): boolean {
   // https://kolibry.zip/guide/api-plugin.html#virtual-modules-convention
