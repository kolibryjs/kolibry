...
export const browserExternalId = '__kolibry-browser-external'
// special id for packages that are optional peer deps
export const optionalPeerDepId = '__kolibry-optional-peer-dep'

const subpathImportsPrefix = '#'

const startsWithWordCharRE = /^\w/

const debug = createDebugger('kolibry:resolve-details', {
...
export interface InternalResolveOptions extends Required<ResolveOptions> {
   ...
   */
   ...
   scan?: boolean
   // Appends ?__kolibry_skip_optimization to the resolved id if shouldn't be optimized
        ...
        = resolveOpts?.custom?.['node-resolve']?.isRequire ?? false
         ...
         if (importer) {
          ...
          || resolveOpts.custom?.depScan?.loader?.startsWith('ts')
               ...
               const moduleLang = this.getModuleInfo(importer)?.meta?.kolibry?.lang
            ...
            if (isBuiltin(id)) {
               if (ssr) {
                ...
                `Module "${id}" has been externalized for browser compatibility, imported by "${importer}". `
                  + 'See http://kolibry.zip/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.',
                     )
                  }
                  return isProduction
                     ? browserExternalId
...
export default new Proxy({}, {
  get(_, key) {
    throw new Error(\`Module "${id}" has been externalized for browser compatibility. Cannot access "${id}.\${key}" in client code.  See http://kolibry.zip/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.\`)
  }
})`
            }
         }
...
export function tryNodeResolve(
   ...
   if (options.ssrOptimizeCheck) {
      return {
         id: skipOptimization
            ? injectQuery(resolved, '__kolibry_skip_optimization')
            : resolved,
      }
   }
