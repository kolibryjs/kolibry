...
export const assetUrlRE = /__KOLIBRY_ASSET__([a-z\d]+)__(?:\$_(.*?)__)?/g
...
export function renderAssetUrlInJS(
   ...
   // imgElement.src = "__KOLIBRY_ASSET__5aa0ddc0__" are using quotes

   // Urls added in CSS that is imported in JS end up like
   // var inlined = ".inlined{color:green;background:url(__KOLIBRY_ASSET__5aa0ddc0__)}\n";

   // In both cases, the wrapping should already be fine

   assetUrlRE.lastIndex = 0
   while ((match = assetUrlRE.exec(code))) {
      s ||= new WizardString(code)
      const [full, referenceId, postfix = ''] = match
      const file = ctx.getFileName(referenceId)
      chunk.kolibryMetadata!.importedAssets.add(cleanUrl(file))
   ...
   // Replace __KOLIBRY_PUBLIC_ASSET__5aa0ddc0__ with absolute paths
...
export const publicAssetUrlRE = /__KOLIBRY_PUBLIC_ASSET__([a-z\d]{8})__/g
...
export function publicFileToBuiltUrl(
   ...
   return `__KOLIBRY_PUBLIC_ASSET__${hash}__`
}
...
async function fileToBuiltUrl(
   ...
   if (
    ...
    || (!file.endsWith('.svg')
      ...
      url = `__KOLIBRY_ASSET__${referenceId}__${postfix ? `$_${postfix}__` : ''}` // TODO_BASE
   }
