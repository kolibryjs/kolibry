 ...
 * dropped.
 */
export const isModernFlag = '__KOLIBRY_IS_MODERN__'
export const preloadMethod = '__kolibryPreload'
export const preloadMarker = '__KOLIBRY_PRELOAD__'
export const preloadBaseMarker = '__KOLIBRY_PRELOAD_BASE__'

export const preloadHelperId = '\0kolibry/preload-helper'
...
function preload(
   ...
   // @ts-expect-error __KOLIBRY_IS_MODERN__ will be replaced with boolean later
   if (!__KOLIBRY_IS_MODERN__ || !deps || deps.length === 0)
      return baseModule()
...
export function buildImportAnalysisPlugin(config: ResolvedConfig): Plugin {
   ...
   // There are three different cases for the preload list format in __kolibryPreload
   //
   // __kolibryPreload(() => import(asyncChunk), [ ...deps... ])
   //
   ...
   const assetsURL = customModulePreloadPaths
      ...
      // a helper `__kolibryPreloadRelativeDep` is used to resolve from relative paths which can be minimized.
      'function(dep, importerUrl) { return dep.startsWith(\'.\') ? new URL(dep, importerUrl).href : dep }'
      : optimizeModulePreloadRelativePaths
         ? // If there isn't custom resolvers affecting the deps list, deps in the list are relative
      // to the current chunk and are resolved to absolute URL by the __kolibryPreload helper itself.
      // The importerUrl is passed as third parameter to __kolibryPreload in this case
         'function(dep, importerUrl) { return new URL(dep, importerUrl).href }'
         : // If the base isn't relative, then the deps are relative to the projects `outDir` and the base
      // is appended inside __kolibryPreload too.
      `function(dep) { return ${JSON.stringify(config.base)}+dep }`
   const preloadCode = `const scriptRel = ${scriptRel};const assetsURL = ${assetsURL};const seen = {};export const ${preloadMethod} = ${preload.toString()}`

   return {
      name: 'kolibry:build-import-analysis',
      resolveId(id) {
                     ...
                     if (url) {
                        ...
                        const addDeps = (filename: string) => {
                           ...
                           if (chunk) {
                              ...
                              chunk.kolibryMetadata!.importedCss.forEach((file) => {
                                 deps.add(file)
                              })
                           }
                           else {
