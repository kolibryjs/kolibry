...
const debug = createDebugger('kolibry:optimize-deps')

export function optimizedDepsPlugin(config: ResolvedConfig): Plugin {
   return {
      name: 'kolibry:optimized-deps',
         ...
         if (depsOptimizer?.isOptimizedDepFile(id)) {
            ...
            if (info) {
               ...
               try {
                  ...
                  // If the refresh has not happened after timeout, Kolibry considers
                  // something unexpected has happened. In this case, Kolibry
...
export function optimizedDepsBuildPlugin(config: ResolvedConfig): Plugin {
   ...
   return {
      name: 'kolibry:optimized-deps-build',
      ...
      async resolveId(id, importer, options) {
         ...
         if (depsOptimizer.isOptimizedDepFile(id)) {
            ...
            if (options?.custom?.['kolibry:pre-alias']) {
