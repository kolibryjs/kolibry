...
export const dynamicImportHelperId = '\0kolibry/dynamic-import-helper'
...
function dynamicImportHelper(glob: Record<string, any>, path: string) {
   const v = glob[path]
   if (v)
...
export function dynamicImportVarsPlugin(config: ResolvedConfig): Plugin {
   ...
   return {
      name: 'kolibry:dynamic-import-vars',
