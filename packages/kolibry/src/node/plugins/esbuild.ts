...
const debug = createDebugger('kolibry:esbuild')
...
export function buildEsbuildPlugin(config: ResolvedConfig): Plugin {
   initTSConfck(config.root)
   ...
   return {
      ...
      // @ts-expect-error injected by @kolibryjs/legacy-plugin
         ...
         if (config.build.lib) {
            ...
            // We don't need to create a WizardString here because both the helpers and
...
async function reloadOnTsconfigChange(changedFile: string) {
   ...
   if (
    ...
    || (changedFile.endsWith('.json')
      ...
      // server may not be available if kolibry config is updated at the same time
      if (server) {
      // force full reload
         server.ws.send({
            type: 'full-reload',
