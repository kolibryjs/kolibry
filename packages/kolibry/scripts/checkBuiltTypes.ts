...
if (errors.length <= 0) {
   consolji.log(colors.nicegreen(colors.bold('passed built types check')))
}
else {
   consolji.log(colors.red(colors.bold('failed built types check')))
   consolji.log('')
   errors.forEach((error) => {
      const pos = error.loc
         ...
         : ''
      consolji.log(
      `${colors.nicecyan(error.file)}:${pos} - importing from ${colors.bold(
...
function collectImportSpecifiers(file: string) {
   ...
   try {
      ...
      consolji.log(colors.red(`failed to parse ${file}`))
      throw e
   }
